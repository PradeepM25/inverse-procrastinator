        ...
        return (
            <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>  
                <div className='bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md'>
                    ...
                    text-sm font-medium hover:bg-red-200 transition-colors'>Retry</button>
                </div>
            </div>
        )
    }
                    ...
                    <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
                        <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
                            ...
                            <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                                <StatCard title='Total Tasks' value={stats.totalTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4
                                text-purple-500' />} />
                                <StatCard title='Completed Tasks' value={stats.completedTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4
                                text-green-500' />} />
                                <StatCard title='Pending Tasks' value={stats.pendingTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4
                                text-fuchsia-500' />} />
                                <StatCard title='Completed Percentage' value={`${stats.completionPercentage}%`} icon={<Zap className='w-3.5 h-3.5 sm:w-4 sm:h-4
                                text-purple-500' />} />
                            </div>
                        ...
                        <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
                            ...
                            <div className='space-y-2 sm:space-y-3'>
                                {tasks.slice(0, 3).map((task) => (
                                    <div key={task._id || task.id} className='flex items-center justify-betweenp-2 sm:p-3 
                                    ...
                                    hover:border-purple-100'>
                                        <div className='flex-1 min-w-0'>
                                            ...
                                            <p className='text-xs text-gray-500 mt-0.5'> 
                                                {task.createdAt ? new Date(task.createdAt).toLocaleDateString(0) : "No Date"}
                                            </p>
                                        </div>
                                        <span className={`px-2 py-1 text-xl rounded-full shrink-0 ml-2
