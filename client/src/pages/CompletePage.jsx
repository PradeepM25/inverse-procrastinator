        ...
        <div className={CT_CLASSES.sortContainer}>
          <div className={CT_CLASSES.sortBox}>
            <div className={CT_CLASSES.filterLabel}>
              <Filter className='w-4 h-4 text-purple-500 sm:w-5 sm:h-5' />
            ...
            <div className={CT_CLASSES.btnGroup}>
              {SORT_OPTIONS.map(opt => (
                ...
                className={
                  [CT_CLASSES.btnBase, sortBy === opt.id ? CT_CLASSES.btnActive : CT_CLASSES.btnInactive].join(" ")
                }>
                  {opt.icon} {opt.label}
                </button>
              ))}
      ...
      <div className={CT_CLASSES.list}>
        {sortedCompletedTasks.length === 0 ? (
          ...
          sortedCompletedTasks.map(task => (
            <TaskItem key={task._id || task.id} task={task} onRefresh={refreshTask} 
              showCompleteCheckBox={false} className='opacity-90 hover:opacity-100 transition-opacity text-sm md:text-base'
            />
          ))
        )}
      </div>
