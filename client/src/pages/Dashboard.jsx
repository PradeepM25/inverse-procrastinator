...
const Dashboard = () => {
  ...
  const filterTask = useMemo(() => tasks.filter(() => {
    ...
    <div className={WRAPPER}>
      ...
      <div className={HEADER}>
        <div className='min-w-0'>
          ...
          <p className='tex-sm text-gray-500 mt-1 ml-1 truncate'>Manage your tasks efficiently</p>
        </div>
          ...
          <div key={key} className={`${STAT_CARD} ${borderColor}`}>
            <div className='flex items-center gap-2 md:gap-3'>
              ...
              <div className='min-w-0'>
                ...
                "bg-gradient-to-br from-fuchsia-500 to-purple-600 bg-clip-text text-transparent" : textColor}`}>{stats[valueKey]}</p>
       ...
       <div className='space-y-6'>
        ...
        <div className='space-y-4'>
            {filterTask.length===0 ? (
              <div className={EMPTY_STATE.wrapper}>
                  ...
                  <p className='text-sm text-gray-500 mb-4'>{filter==="all" ? "Create your first task to get started"
                  : "No tasks matched this filter"}</p>
                  <button onClick={() => setShowModal(true)} className={EMPTY_STATE.btn}> Add New Task</button>
                </div>
            ) : (
              filterTask.map(task => (
                <TaskItem key={task._id || task.id} task={task} onRefresh={refreshTasks}
                  showCompleteCheckBox onEdit={() => {selectedTask(task); setShowModal(true)}}
                />
              ))
            )}
        </div>
        ...
        <div onClick={() => setShowModal(true)} 
         ...
         hover:border-purple-400 bg-purple-50/50 cursor-pointer transition-colors'>
          ...
          <span className='text-gray-600 fotn-medium'> Add Task</span>
        </div>
       </div>
