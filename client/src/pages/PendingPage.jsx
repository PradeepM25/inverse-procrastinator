      ...
      <div className={layoutClasses.headerWrapper}>
        <div>
          ...
          <p className='text-sm text-gray-500 mt-1 ml-1'>
            {sortedPendigTasks.length} task{sortedPendigTasks.length !== 1 && 's'}
            {' '}Needing your attenction 
          </p>
        </div>

        <div className={layoutClasses.sortBox}>
          <div className='flex items-center gap-2 text-shadow-gray-700 font-medium'>
        ...
        {sortedPendigTasks.length === 0 ? (
          <div className={layoutClasses.emptyState}>
            <div className='max-w-xs mx-auto py-6'>
              ...
              <p className='text-sm text-gray-500 mb-4'> No Pending Tasks - Keep it up</p>
              <button onClick={() => setShowModal(true)} className={layoutClasses.emptyBtn}> Create new task</button>
            </div>
          </div>
        ) : (
          sortedPendigTasks.map(task => (
